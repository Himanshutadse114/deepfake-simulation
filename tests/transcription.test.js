...
test('rejects extra generated speech', () => {
  const result = compareTranscripts(
    ...
    'This is the complete approved message for the simulation and here are several unrelated extra words.'
  );
  assert.equal(result.matches, false);
  assert.ok(result.actualWordCount > result.expectedWordCount);
});

test('reads the transcription returned by the pinned Whisper model', () => {
